...
Stories call `SaudiNationalDayCarousel.mount()` after loading the published UMD from **jsDelivr** (default) or **unpkg**. One story per Figma option. The **Portal template** stories compose a home page inspired by [dev-dga-templates](https://github.com/DevDhaif/dev-dga-templates) and replace that template's hero with this carousel. Independent, not affiliated with the DGA.
